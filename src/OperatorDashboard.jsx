...
import {
  ...
  faPaperPlane,
  faCheck,
} from "@fortawesome/free-solid-svg-icons"; // Added faCheck
...
const baseStyles = {
  // ... (Apka purana styles waisa hi hai)
  dashboardContainer: {
    display: "flex",
    minHeight: "100vh",
  ...
  notificationHeader: {
    backgroundColor: "#452983", // Dark theme matching your screenshot
    color: "white",
    padding: "12px 15px",
    fontSize: "0.95rem",
    fontWeight: "600",
...
function OperatorDashboard() {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://threebapi-1067354145699.asia-south1.run.app/api/items/items/employee/${employeeId}?lang=${selectedLang}`,
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    fetch(
      `https://threebapi-1067354145699.asia-south1.run.app/api/workers/employee-task/${employeeId}?lang=${selectedLang}`,
    )
      .then((res) => res.json())
      .then((data) => setOngoingTasks(data?.data?.length || 0))
      .catch((err) => console.error(err));
    fetch(
      `https://threebapi-1067354145699.asia-south1.run.app/api/task-transfers/transfers?lang=${selectedLang}`,
    )
      .then((res) => res.json())
              ...
              <div style={baseStyles.notificationDropdown}>
                ...
                <div style={{ maxHeight: "300px", overflowY: "auto" }}>
                  ...
                  >
                    ...
                    <div>
                      <p style={baseStyles.notificationTitle}>{latestMsg}</p>
                      <span style={baseStyles.notificationTime}>Recently</span>
                    </div>
                  </div>
                </div>
                <div
