...
const AddAssignmentModal = ({ show, onClose, onSubmitSuccess, initialData }) => {
  ...
  const generateUniqueKey = (data) => {
    ...
    if (!mName || mName === 'N/A') mName = localStorage.getItem("currentMachineNo");
    if (!iName || iName === 'N/A') iName = localStorage.getItem("currentItemNo");
  ...
  useEffect(() => {
    ...
    const finalItemName = (initialData?.itemName && initialData.itemName !== 'N/A')
      ? initialData.itemName : storedItemNo;

    const finalMachineNumber = (initialData?.machineNumber && initialData.machineNumber !== 'N/A')
      ? initialData.machineNumber : storedMachineNo;
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content p-3">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-2">
              ...
              <select id="time" className="form-select" value={formData.time} onChange={handleChange}>
                {timeSlots.map((t) => {
                  ...
                  return (
                    <option key={t} value={t} disabled={isDisabled} style={isDisabled ? { color: "#ccc" } : {}}>
                      {t} {isDisabled ? "(Booked)" : ""}
                    </option>
                  );
                })}
            ...
            <div className="mb-2">
              <label>Shift</label>
              <select id="shift" className="form-select" value={formData.shift} onChange={handleChange} required>
            ...
            <div className="mb-2">
              ...
              <input type="number" id="numberOfBox" className="form-control" value={formData.numberOfBox} onChange={handleChange} required />
            </div>
            ...
            <div className="mb-3 border p-2 rounded bg-light">
              <label className="form-label fw-bold text-danger">Take Selfie *</label>
              <input type="file" accept="image/*" capture="user" className="form-control" onChange={handleSelfieChange} required />
              {preview && <div className="mt-2 text-center"><img src={preview} alt="Prev" className="img-thumbnail" style={{ maxHeight: "150px" }} /></div>}
            </div>

            <button type="submit" className="btn btn-dark w-100" disabled={isSubmitting || bookedSlots.includes(formData.time)}>
              {isSubmitting ? "Saving..." : "Save"}
            </button>
          </form>

          <button className="btn btn-outline-secondary mt-2 w-100" onClick={onClose}>Close</button>
        </div>
      </div>
    </div>
  );
