...
function AssignmentsPage() {
  ...
  const handleOpenMixtureEditModal = (assignment) => {
    ...
    const mixtureId = assignment?.mixtures?.[0]?._id || ""; // First mixture id if available
    ...
    localStorage.setItem(
      "initialMixtureForm",
      JSON.stringify(initialFormDataForMixture),
    );
  ...
  if (loading)
    return (
      ...
      >
        ...
        />
        <p style={{ marginTop: "15px", fontWeight: "bold", color: "#452983" }}>
  ...
  if (!employeeId)
    return (
      <p style={{ padding: "20px" }}>No employee data found. Please login.</p>
    );
  ...
  return (
    <div style={baseStyles.pageContainer}>
      ...
      >{`Assignments for ${employeeName || "Employee"}`}</h1>
            ...
            <div key={assignment._id} style={baseStyles.assignmentCard}>
              <div>
                ...
                <p style={baseStyles.cardRow}>
                  <strong>Operator Machine Number:</strong>{" "}
                  {assignment.machineNumber || "N/A"}
                </p>
                <p style={baseStyles.cardRow}>
                  <strong>Length:</strong> {assignment.length || "N/A"}
                </p>
                <p style={baseStyles.cardRow}>
                  <strong>No. of Sticks:</strong>{" "}
                  {assignment.noOfSticks || "N/A"}
                </p>

                <div style={baseStyles.cardRow}>
                  <strong>Product Description:</strong> Power Cut
                </div>
                ...
                <p style={baseStyles.cardRow}>
                  ...
                  {assignment.mixtures && assignment.mixtures.length > 0
                ...
                <p style={baseStyles.cardRow}>
                  ...
                  {assignment.helpers && assignment.helpers.length > 0
                    ? assignment.helpers[0].name
                    : "N/A"}
                </p>
                <p style={baseStyles.cardRow}>
                  <strong>Shift:</strong> {assignment.shift || "N/A"}
                </p>
                <p style={baseStyles.cardRow}>
                  <strong>Company:</strong> {assignment.company || "N/A"}
                </p>
              </div>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (role === "Operator" || role === "Helper") {
                      ...
                      console.log(
                        "Calling handleOpenMixtureEditModal() for Mixture",
                      );
                      handleOpenMixtureEditModal(assignment);
                    } else {
                      console.log(
                        "No matching role found in localStorage for Edit Assignment.",
                      );
                    }
                  }}
                >
